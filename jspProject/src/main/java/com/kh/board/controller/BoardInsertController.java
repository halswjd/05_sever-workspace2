...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) { // multipart/form-data 잘 입력했으면 true 반환
			
			// 파일 업로드를 위한 라이브러리 : cos.jar(com.oreilly.servlet의 약자)
			// http://www.servlets.com
			
			// 1. 전달되는 파일 처리할 작업 내용 (전달되는 파일의 용량제한, 전달된 파일을 저장시킬 폴더 경로)
			// 1_1) 전달되는 파일의 용량 제한 (int maxSize => byte 단위로 맞춰서) => 10Mbyte
			/*
			 ...
			 * 
			 * 		단, 업로드시 파일명은 수정해주는게 일반적임 그래서 파일명 수정시켜주는 객체 제시행됨
			 ...
			 * 		=> 내부적으로 해당 클래스에서 rename() 메소드 실행되면서 파일명 수정된 후 업로드
			 * 		   rename(원본파일) {
			 * 				기존에 동일한 파일명이 존재하는지 검사함
			 * 				존재할 경우 파일명 뒤에 카운팅 된 숫자를 붙여줌
			 * 				ex) aaa.jpg, aaa1.jpg, aaa2.jpg, ...
			 * 					꽃.png, 꽃1.png
			 * 				단점 : 덮어쓰기는 방지하지만 파일명에 한글/특문/띄어쓰기는 잡아주지 못함
			 * 				return 수정파일
			 * 		   }
			 * 
			 * 
			 * 		나만의 방식대로 절대 안겹치도록 rename 할 수 있게 나만의 FileRenamePolicy 클래스 만들기!!(rename 메소드 재정의!)
			 * 		com.kh.common.MyFileRenmaePolicy 클래스 만들기
			 */
...
//			MultipartRequest multiRequest = new MultipartRequest(request, 저장시킬폴더의경로, 용량제한, 인코딩값, 파일명수정시켜주는객체);
			...
			// > Board : 카테고리번호, 제목, 내용, 작성자 회원번호 뽑아서 BOARD테이블에 INSERT
			
			// > Attachment : 넘어온 첨부파일 있으면 원본명, 수정명, 저장폴더경로 뽑아서 ATTACHMENT 테이블에 INSERT
			...
			// multiRequest.getOriginalFileName("키") : 넘어온 파일이 있을 경우 "원본명" | 없을 경우 null 반환
